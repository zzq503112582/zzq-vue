<template>
  <div>
    ...
    <el-card>
      ...
      :current-page="querIfno.pagenum"
      :page-sizes="[3, 5, 10, 15]"
      :page-size="querIfno.pagesize"
...
export default {
  data() {
    return {
      ...
      addCateFromRules:{
        cat_name:[{
          required:true,message:'请输入分类名称',triggger:'blur'
        }]
      },
  //父级分类的列表
    parentCateList:[],
  ...
  methods: {
    ...
    handleSizeChange(newSize){
      this.querIfno.pagesize =newSize
      this.getCateList()
    },
    //监听pagenum改变
    handleCurrentChange(newPage){
      this.querIfno.pagenum=newPage
      this.getCateList()
    },
    // 控制添加分类的显示与隐藏
    showAddCateDialog(){
  ...
  parentCateChanged(){
    ...
    //如果selectedKeys数组中的lebgth大于0 证明选中的是父级分类
    //反之就没选中
